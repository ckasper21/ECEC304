...
#include "cyapicallbacks.h"
    ...
    /*******************************************************************************
    * Function Name: I2C_EZI2C_STRETCH_ISR
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    CY_ISR_PROTO(I2C_EZI2C_STRETCH_ISR)
    {
    ...
    #ifdef I2C_EZI2C_STRETCH_ISR_ENTRY_CALLBACK
        I2C_EZI2C_STRETCH_ISR_EntryCallback();
    #endif /* I2C_EZI2C_STRETCH_ISR_ENTRY_CALLBACK */

    #if(I2C_SECONDARY_ADDRESS_ENABLE_CONST)
        /* Variable intended to be used with either buffer */
        static volatile uint8 * I2C_dataBuffer; /* Pointer to data buffer              */
        static uint16 I2C_bufSizeBuf;           /* Size of buffer in bytes             */
        static uint16 I2C_protectBuf;           /* Start index of write protected area */

        static uint8 activeAddress;
        uint32 ackResponse;

        ackResponse = I2C_EZI2C_ACK_RECEIVED_ADDRESS;
    ...
    #else
        CY_I2C_CUSTOM_INTR_HANDLER();
    #endif /* !defined (CY_REMOVE_I2C_CUSTOM_INTR_HANDLER) */

        /* Make local copy of global variable */
        locIndex = I2C_EZI2C_GET_INDEX(activeAddress);

        /* Get interrupt sources */
        locIntrSlave = I2C_GetSlaveInterruptSource();
        locIntrCause = I2C_GetInterruptCause();
        ...
        */
        if(0u != (locIntrSlave & (I2C_INTR_SLAVE_I2C_ARB_LOST |
                                  I2C_INTR_SLAVE_I2C_BUS_ERROR)))
        {
            I2C_CTRL_REG &= (uint32) ~I2C_CTRL_ENABLED; /* Disable SCB block */

        #if(I2C_CY_SCBIP_V0)
            if(0u != ((uint8) I2C_EZI2C_STATUS_BUSY & I2C_curStatus))
        #endif /* (I2C_CY_SCBIP_V0) */
            {
                I2C_curStatus &= (uint8) ~I2C_EZI2C_STATUS_BUSY;
                I2C_curStatus |= (uint8)  I2C_EZI2C_STATUS_ERR;

                /* INTR_TX_EMPTY is enabled in the address phase to receive data */
                if(0u == (I2C_GetTxInterruptMode() & I2C_INTR_TX_EMPTY))
                {
                    /* Write complete */
                    if(I2C_indexBuf1 != I2C_offsetBuf1)
                    {
                        I2C_curStatus |= (uint8) I2C_INTR_SLAVE_I2C_WRITE_STOP;
                    }
                }
                else
                {
                    /* Read complete */
                    I2C_curStatus |= (uint8) I2C_INTR_SLAVE_I2C_NACK;
                }
            }
            ...
            I2C_CTRL_REG |= (uint32) I2C_CTRL_ENABLED;  /* Enable SCB block */
        }
        else
        {
            /* INTR_I2C_EC_WAKE_UP */
            /* Wakes up device from deep sleep */
            if(0u != (locIntrCause & I2C_INTR_CAUSE_I2C_EC))
            {
            ...
            if(0u != (locIntrCause & (I2C_INTR_CAUSE_RX | I2C_INTR_CAUSE_SLAVE)))
            {
                /* INTR_RX.NOT_EMPTY */
                /* Receives data byte-by-byte. Does not use RX FIFO capabilities */
                if (0u != (I2C_GetRxInterruptSourceMasked() & I2C_INTR_RX_NOT_EMPTY))
                {
                #if(I2C_SECONDARY_ADDRESS_ENABLE_CONST)
                    ...
                    */
                    if (0u == (locIntrCause & I2C_INTR_CAUSE_SLAVE))
                #endif /* (I2C_SECONDARY_ADDRESS_ENABLE_CONST) */
                    {
                        locByte = I2C_RX_FIFO_RD_REG;

                        switch(I2C_fsmState)
                        {
                        case I2C_EZI2C_FSM_BYTE_WRITE:
                            if(0u != locBufSize)
                            {
                                /* Store data byte and ACK */
                                I2C_I2C_SLAVE_GENERATE_ACK;

                                I2C_dataBuffer1[locIndex] = (uint8) locByte;
                                locIndex++;
                                locBufSize--;
                            }
                            else
                    ...
                    #if(I2C_SUB_ADDRESS_SIZE16_CONST)
                        case I2C_EZI2C_FSM_OFFSET_HI8:
                            ...
                            I2C_fsmState = I2C_EZI2C_FSM_OFFSET_LO8;

                            break;
                    #endif /* (I2C_SUB_ADDRESS_SIZE16_CONST) */

                        case I2C_EZI2C_FSM_OFFSET_LO8:

                            #if (I2C_SUB_ADDRESS_SIZE16)
                            {
                            ...
                            if(locByte < (uint32) I2C_bufSizeBuf1)
                            {
                                I2C_I2C_SLAVE_GENERATE_ACK;
                                ...
                                locBufSize = (uint16) ((locByte < I2C_protectBuf1) ?
                                                       (I2C_protectBuf1 - locByte) : (0u));

                            #if(I2C_CY_SCBIP_V0)

                                if(locBufSize < I2C_EZI2C_FIFO_SIZE)
                                {
                                    /* Set FSM state to receive byte by byte */
                                    I2C_fsmState = I2C_EZI2C_FSM_BYTE_WRITE;
                                }
                                /* Receive RX FIFO chunks */
                                else if(locBufSize == I2C_EZI2C_FIFO_SIZE)
                                {
                                    I2C_ENABLE_SLAVE_AUTO_DATA; /* NACK when RX FIFO is full */
                                    I2C_SetRxInterruptMode(I2C_NO_INTR_SOURCES);
                                }
                                else
                                {
                                    I2C_ENABLE_SLAVE_AUTO_DATA_ACK; /* Stretch when RX FIFO is full */
                                ...
                                I2C_EZI2C_SET_OFFSET(activeAddress, locIndex);
                            }
                            else
                            {
                ...
                else if (0u != (I2C_I2C_CTRL_REG & I2C_I2C_CTRL_S_READY_DATA_ACK))
                {
                    ...
                    */
                    if(0u != (locIntrCause & I2C_INTR_CAUSE_SLAVE))
                    {
                        ...
                        fifoIndex = I2C_GET_RX_FIFO_ENTRIES;
                        ...
                        #if(I2C_SECONDARY_ADDRESS_ENABLE)
                        {
                            ...
                            if(I2C_CHECK_INTR_SLAVE_MASKED(I2C_INTR_SLAVE_I2C_ADDR_MATCH))
                            {
                                ...
                                */
                                fifoIndex -= ((0u != I2C_GET_RX_FIFO_SR_VALID) ? (0u) : (1u));
                            }
                        }
                        #endif

                        I2C_DISABLE_SLAVE_AUTO_DATA;
                        endTransfer = I2C_EZI2C_CONTINUE_TRANSFER;
                    }
                    else
                    /* INTR_RX_FULL */
                    /* Continue transfer or disable INTR_RX_FULL to catch completion event. */
                    {
                        ...
                        locBufSize -= (uint16) I2C_EZI2C_FIFO_SIZE;

                        if(locBufSize <= I2C_EZI2C_FIFO_SIZE)
                        {
                            /* Send NACK when RX FIFO overflow */
                            fifoIndex   = locBufSize;
                            endTransfer = I2C_EZI2C_COMPLETE_TRANSFER;
                        }
                        else
                        {
                            /* Continue  transaction */
                            fifoIndex   = I2C_EZI2C_FIFO_SIZE;
                            endTransfer = I2C_EZI2C_CONTINUE_TRANSFER;
                        }
                    }
                    ...
                    for(; (0u != fifoIndex); fifoIndex--)
                    {
                        ...
                        I2C_dataBuffer1[locIndex] = (uint8) I2C_RX_FIFO_RD_REG;
                        locIndex++;
                    }

                    /* Complete transfer sending NACK when RX FIFO overflow */
                    if(I2C_EZI2C_COMPLETE_TRANSFER == endTransfer)
                    {
            ...
            if(0u != (locIntrCause & I2C_INTR_CAUSE_SLAVE))
            {
            ...
            #if(!I2C_SECONDARY_ADDRESS_ENABLE_CONST)
                if(0u != (locIntrSlave & I2C_INTR_SLAVE_I2C_STOP))
            #else
                /* Prevent triggering when matched address was NACKed */
                if((0u != (locIntrSlave & I2C_INTR_SLAVE_I2C_STOP)) &&
                   (0u != ((uint8) I2C_EZI2C_STATUS_BUSY & I2C_curStatus)))
            #endif
                {
                    /* Disable TX and RX interrupt sources */
                    I2C_SetRxInterruptMode(I2C_NO_INTR_SOURCES);
                    ...
                    locStatus = I2C_INTR_SLAVE_I2C_NACK;

                    /* Check if buffer content was modified: the address phase resets the locIndex */
                    if(locIndex != I2C_EZI2C_GET_OFFSET(activeAddress))
                    {
                        locStatus |= I2C_INTR_SLAVE_I2C_WRITE_STOP;
                    }

                    /* Complete read or write transaction */
                    locStatus &= locIntrSlave;
                    I2C_EZI2C_UPDATE_LOC_STATUS(activeAddress, locStatus);
                    locStatus |= (uint32)  I2C_curStatus;
                    locStatus &= (uint32) ~I2C_EZI2C_STATUS_BUSY;
                    I2C_curStatus = (uint8) locStatus;
                    ...
                    #if(I2C_SECONDARY_ADDRESS_ENABLE)
                    {
                        ...
                        I2C_EZI2C_SET_INDEX(activeAddress, locIndex);
                    }
                    #endif
                }
                ...
                */
                if(0u != (locIntrSlave & I2C_INTR_SLAVE_I2C_ADDR_MATCH))
                {
                    #if(I2C_SECONDARY_ADDRESS_ENABLE)
                    {
                        /* Read address byte from RX FIFO */
                        locByte = I2C_GET_I2C_7BIT_ADDRESS(I2C_RX_FIFO_RD_REG);

                        I2C_ClearRxInterruptSource(I2C_INTR_RX_NOT_EMPTY);

                        /* Check received address against device addresses */
                        if(I2C_addrBuf1 == locByte)
                        {
                            /* Set buffer exposed to primary slave address */
                            I2C_dataBuffer = I2C_dataBuffer1;
                            I2C_bufSizeBuf = I2C_bufSizeBuf1;
                            I2C_protectBuf = I2C_protectBuf1;

                            activeAddress = I2C_EZI2C_ACTIVE_ADDRESS1;
                        }
                        else if(I2C_addrBuf2 == locByte)
                        {
                            /* Set buffer exposed to secondary slave address */
                            I2C_dataBuffer = I2C_dataBuffer2;
                            I2C_bufSizeBuf = I2C_bufSizeBuf2;
                            I2C_protectBuf = I2C_protectBuf2;

                            activeAddress = I2C_EZI2C_ACTIVE_ADDRESS2;
                        }
                        else
                        {
                            /* Address does not match */
                            ackResponse = I2C_EZI2C_NACK_RECEIVED_ADDRESS;
                        }
                    }
                    #endif

                #if(I2C_SECONDARY_ADDRESS_ENABLE_CONST)
                    if(I2C_EZI2C_NACK_RECEIVED_ADDRESS == ackResponse)
                    {
                        ...
                        I2C_DISABLE_INTR_SLAVE(I2C_INTR_SLAVE_I2C_STOP);
                    #endif /* (!I2C_CY_SCBIP_V0) */
                ...
                #endif /* (I2C_SECONDARY_ADDRESS_ENABLE_CONST) */
                    {
                        ...
                        I2C_ENABLE_INTR_SLAVE(I2C_INTR_SLAVE_I2C_STOP);
                    #endif /* (!I2C_CY_SCBIP_V0) */
                ...
                I2C_ClearSlaveInterruptSource(locIntrSlave);
            }
            ...
            */
            if(0u != (I2C_GetInterruptCause() & I2C_INTR_CAUSE_TX))
            {
                ...
                do
                {
                    ...
                    if(0u != locBufSize)
                    {
                        I2C_TX_FIFO_WR_REG = (uint32) I2C_dataBuffer1[locIndex];
                        locIndex++;
                        locBufSize--;
                    }
                    else
                    {
                        I2C_TX_FIFO_WR_REG = I2C_EZI2C_OVFL_RETURN;
                    }
                }
                while(I2C_EZI2C_FIFO_SIZE != I2C_GET_TX_FIFO_ENTRIES);
    ...
    #ifdef I2C_EZI2C_STRETCH_ISR_EXIT_CALLBACK
        I2C_EZI2C_STRETCH_ISR_ExitCallback();
    #endif /* I2C_EZI2C_STRETCH_ISR_EXIT_CALLBACK */
    ...
    /*******************************************************************************
    * Function Name: I2C_EZI2C_NO_STRETCH_ISR
    ****************************************************************************//**
    *
    *  Handles the Interrupt Service Routine for the SCB EZI2C mode.
    ...
    *******************************************************************************/
    ...
    #ifdef I2C_EZI2C_NO_STRETCH_ISR_ENTRY_CALLBACK
        I2C_EZI2C_NO_STRETCH_ISR_EntryCallback();
    #endif /* I2C_EZI2C_NO_STRETCH_ISR_ENTRY_CALLBACK */
        ...
        */
                                  ...
                                  I2C_INTR_SLAVE_I2C_BUS_ERROR)))
        {
        ...
        #endif /* (I2C_CY_SCBIP_V0) */
            {
                ...
                if(0u != (I2C_EZI2C_FSM_WRITE_MASK & I2C_fsmState))
                {
                    /* Write complete */
                    if(I2C_indexBuf1 != I2C_offsetBuf1)
                    {
        ...
        else
        {
            ...
            */
            if(0u != (locIntrCause & I2C_INTR_CAUSE_RX))
            {
                ...
                case I2C_EZI2C_FSM_BYTE_WRITE:
                    ...
                    if(I2C_indexBuf1 < I2C_protectBuf1)
                    {
                        /* Stores received byte into buffer */
                        I2C_dataBuffer1[I2C_indexBuf1] = (uint8) locByte;
                        I2C_indexBuf1++;
                    }
                    else
                    {
                        /* Discard current byte and sets FSM state to discard following bytes */
                        I2C_fsmState = I2C_EZI2C_FSM_WAIT_STOP;
                    }
                ...
                case I2C_EZI2C_FSM_OFFSET_LO8:
                    ...
                    if(locByte < (uint32) I2C_bufSizeBuf1)
                    {
                        ...
                        I2C_offsetBuf1 = (uint8) locByte;
                        I2C_indexBuf1  = (uint16) locByte;
                    ...
                    else
                    {
                        ...
                        I2C_indexBuf1 = (uint16) I2C_offsetBuf1;
            ...
            if(0u != (locIntrSlave & I2C_INTR_SLAVE_I2C_START))
            {
            #if(!I2C_CY_SCBIP_V0)
                ...
                #else
                {
                    ...
                    I2C_ClearSlaveInterruptSource(I2C_INTR_SLAVE_I2C_ADDR_MATCH);
                }
                #endif
            #else
                /* Manage INTR_SLAVE.I2C_ADDR_MATCH as slave busy status */
                ...
                I2C_FAST_CLEAR_TX_FIFO;
                I2C_TX_FIFO_WR_REG = (uint32) I2C_dataBuffer1[I2C_offsetBuf1];

                /* Store buffer index to be handled by INTR_SLAVE.I2C_STOP */
                locByte = (uint32) I2C_indexBuf1;

                /* Update index: one byte is already in the TX FIFO */
                I2C_indexBuf1 = (uint16) I2C_offsetBuf1 + 1u;

                /* Enable INTR_TX.NOT_FULL to load TX FIFO */
                I2C_SetTxInterruptMode(I2C_INTR_TX_TRIGGER);

                /* Clear locIntrSlave after INTR.TX_TRIGGER is enabled */
                I2C_ClearSlaveInterruptSource(locIntrSlave);

                locIntrCause |= I2C_INTR_CAUSE_TX;
            }


            /* INTR_TX.TRIGGER */
            /* Transmits data to the master: loads data into the TX FIFO. The TX FIFO is loaded with data
            *  until used entries are less than I2C_TX_LOAD_SIZE. If index reaches end of the
            *  buffer the 0xFF is sent to the end of transfer.
            */
            if(0u != (locIntrCause & I2C_INTR_CAUSE_TX))
            {
                /* Put data into TX FIFO until there is room */
                do
                {
                    /* Check transmit buffer range */
                    if(I2C_indexBuf1 < I2C_bufSizeBuf1)
                    {
                        I2C_TX_FIFO_WR_REG = (uint32) I2C_dataBuffer1[I2C_indexBuf1];
                        I2C_indexBuf1++;
                    }
                    else
                    {
                ...
                while(I2C_TX_LOAD_SIZE != I2C_GET_TX_FIFO_ENTRIES);

                I2C_ClearTxInterruptSource(I2C_INTR_TX_TRIGGER);
            }
            ...
            if(0u != (locIntrSlave & I2C_INTR_SLAVE_I2C_STOP))
            {
                ...
                if((locByte != I2C_offsetBuf1) &&
                   (0u != (I2C_EZI2C_FSM_WRITE_MASK & I2C_fsmState)))
                {
                ...
                I2C_curStatus |= (uint8) (locStatus & locIntrSlave);
            ...
            #if(I2C_EZI2C_EC_AM_ENABLE)
            {
                ...
                if(0u != (locIntrCause & I2C_INTR_CAUSE_I2C_EC))
                {
                    I2C_I2C_SLAVE_GENERATE_NACK; /* NACK in active mode */
                    I2C_ClearI2CExtClkInterruptSource(I2C_INTR_I2C_EC_WAKE_UP);
                }
            }
            #endif
    ...
    #ifdef I2C_EZI2C_NO_STRETCH_ISR_EXIT_CALLBACK
        I2C_EZI2C_NO_STRETCH_ISR_ExitCallback();
    #endif /* I2C_EZI2C_NO_STRETCH_ISR_EXIT_CALLBACK */

    }
#endif /* (I2C_EZI2C_SCL_STRETCH_DISABLE_CONST) */
